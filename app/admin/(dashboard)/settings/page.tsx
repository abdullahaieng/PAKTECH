...
import { api, type StoreSettings } from "@/lib/admin/api";
...
export default function AdminSettingsPage() {
  ...
  if (!settings) return <p className="text-[var(--admin-muted)]">Loading...</p>;

  return (
    <div>
      <div className="mb-8">
        <h1 className="text-2xl font-bold">Settings</h1>
        <p className="text-[var(--admin-muted)] mt-1">Store shipping and general settings</p>
      </div>
      ...
      >
        ...
        {passwordMsg && <p className="text-xs text-[var(--admin-muted)]">{passwordMsg}</p>}
      </form>
    </div>
  );
}
