    ...
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-[var(--admin-muted)] mt-1">{orders.length} total orders</p>
        </div>
        <select
          value={filter}
      ...
      <div className="bg-white rounded-xl border border-[var(--admin-border)] overflow-hidden">
        {loading ? (
          ...
          <p className="p-8 text-center text-[var(--admin-muted)]">No orders found</p>
        ) : (
          <table className="w-full text-sm">
            <thead className="bg-slate-50 border-b">
              <tr>
                <th className="text-left p-4 font-medium text-[var(--admin-muted)]">Order</th>
                <th className="text-left p-4 font-medium text-[var(--admin-muted)]">Customer</th>
                <th className="text-left p-4 font-medium text-[var(--admin-muted)]">Date</th>
                <th className="text-left p-4 font-medium text-[var(--admin-muted)]">Total</th>
                <th className="text-left p-4 font-medium text-[var(--admin-muted)]">Status</th>
              ...
              {filtered.map((order) => (
                <tr key={order.id} className="hover:bg-slate-50">
                  ...
                  <td className="p-4">
                    ...
                    <p className="text-xs text-[var(--admin-muted)]">{order.shippingAddress.email}</p>
                  </td>
                  ...
                  <td className="p-4">
                    <div className="flex items-center justify-end gap-2">
                      <Link href={`/admin/orders/${order.id}`} className="p-1.5 rounded hover:bg-slate-100 text-[var(--admin-muted)]">
