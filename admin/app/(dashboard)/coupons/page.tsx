    ...
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-[var(--muted)] mt-1">Create and manage discount codes</p>
        </div>
        <button
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-white rounded-xl border p-6 max-w-md mb-6 space-y-4">
          ...
          <div className="flex gap-3">
            ...
            <button type="button" onClick={resetForm} className="px-4 py-2 text-sm text-[var(--muted)] hover:text-foreground">
              Cancel
            </button>
          </div>
        </form>
      ...
      <div className="space-y-3 max-w-2xl">
        {loading ? (
          ...
          <p className="text-[var(--muted)]">No coupons yet — create one above</p>
        ) : (
          coupons.map((coupon) => (
