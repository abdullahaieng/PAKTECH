...
export default function OrderDetailPage() {
  ...
  if (!order) return <p className="text-[var(--muted)]">Loading order...</p>;

  const addr = order.shippingAddress;

  return (
    <div>
      <Link href="/orders" className="flex items-center gap-2 text-sm text-[var(--muted)] hover:text-foreground mb-6">
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white rounded-xl border p-5">
            ...
            <div className="border-t mt-4 pt-4 space-y-2 text-sm">
              <div className="flex justify-between"><span className="text-[var(--muted)]">Subtotal</span><span>{formatPrice(order.subtotal)}</span></div>
              <div className="flex justify-between"><span className="text-[var(--muted)]">Shipping</span><span>{order.shipping === 0 ? "FREE" : formatPrice(order.shipping)}</span></div>
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-xl border p-5">
            ...
            <p className="text-sm text-[var(--muted)]">{addr.email}</p>
