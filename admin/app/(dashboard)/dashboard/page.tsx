...
export default function DashboardPage() {
  ...
  if (!stats) return <p className="text-red-500">Failed to load stats. Is the store running?</p>;

  return (
    <div>
      <div className="mb-8">
        <h1 className="text-2xl font-bold">Dashboard</h1>
        <p className="text-[var(--muted)] mt-1">Store overview and recent activity</p>
      </div>
      ...
      {stats.lowStockProducts > 0 && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-6 flex items-center gap-3">
          ...
          <p className="text-sm text-amber-800">
            ...
            <Link href="/products" className="underline font-medium">Check products</Link>
          </p>
        </div>
      )}
      ...
      <div className="bg-white rounded-xl border border-[var(--border)]">
        ...
        {stats.recentOrders.length === 0 ? (
          <p className="p-8 text-center text-[var(--muted)]">No orders yet</p>
        ) : (
            ...
            {stats.recentOrders.map((order) => (
              ...
              >
                <div>
                  ...
                  <p className="text-xs text-[var(--muted)]">{order.shippingAddress.firstName} {order.shippingAddress.lastName}</p>
                </div>
